...
;(function ( $ ) {
    ...
    // Toast on Click
    const toastTrigger = document.getElementById('toasttr')
    const toastLiveExample = document.getElementById('toastMsg')
    if (toastTrigger) {
...
$(function() {
    ...
    var end = moment();
    var getrangeTxt = '';
    function cb(start, end, getrangeTxt) {
        $('.rangePicker').html(start.format('D MMM YYYY') + ' - ' + end.format('D MMM YYYY'));
        $('.txtfield').html(getrangeTxt);
    }
    ...
    $('.daterangepicker > .ranges li').on('click',function(){
        var getrangeTxt = $(this).attr('data-range-key');
        return getrangeTxt;
    });
});