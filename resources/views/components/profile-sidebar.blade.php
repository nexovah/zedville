...
<aside
    class="fixed inset-y-0 z-10 flex flex-col flex-shrink-0 w-72 max-h-screen overflow-hidden transition-all transform border-r-[2px] border-color-[#E8E8EA] bg-[#EEF9F5] lg:z-auto lg:static {{ $sidebarOpen ? '' : '-translate-x-full lg:translate-x-0 lg:w-20' }}"
    :class="{'-translate-x-full lg:translate-x-0 lg:w-20': !isSidebarOpen}">
    <!-- sidebar header -->
    <div class="flex items-center justify-between flex-shrink-0 {{ $sidebarOpen ? '' : 'lg:justify-center' }}" :class="{'lg:justify-center': !isSidebarOpen}">
        <div class="py-4 px-[30px] w-full {{ $sidebarOpen ? '' : 'px-[12px]' }}" :class="{'px-[12px]': !isSidebarOpen}">
            <div class="sidebarLogo {{ $sidebarOpen ? '' : 'collaspeSidebar' }}" :class="{'collaspeSidebar': !isSidebarOpen}">
    ...
    <!-- Sidebar links -->
    <div class="userDtls px-[22px] mt-[8px] xl:mt-[16px] 2xl:mt-[30px] {{ $sidebarOpen ? '' : 'px-[12px]' }}" :class="{'px-[12px]': !isSidebarOpen}">
        <div class="flex items-center justify-between rounded-md bg-[#56F4CF]/10 p-1 xl:p-2 2xl:p-3 border-2 border-[#56F4CF]" :class="{'p-1': !isSidebarOpen}">
            ...
            <a href="{{ route('profile.edit') }}" class="sidebarUserSection flex items-center relative gap-x-2 w-full justify-between">
                <div class="userImg relative">
                    <img src="{{ asset('asset/front/images/' . $selectedAvatar->name)}}" class="w-12 h-12 rounded-full object-cover {{ $sidebarOpen ? '' : 'w-8 h-8' }}" :class="{'w-8 h-8': !isSidebarOpen}" alt="User Avatar">
                    <span class="absolute bottom-[4px] 2xl:bottom-[-10px] xl:bottom-[-4px] right-[-6px] 2xl:right-[-10px] xl:right-[-6px] {{ $sidebarOpen ? '' : 'bottom-0 right-0 w-4' }}" :class="{'bottom-0 right-0 w-4': !isSidebarOpen}">
                        <img src="{{ asset('asset/front/images/label1.svg')}}" class="{{ $sidebarOpen ? '' : 'w-4' }} w-4 xl:w-5 2xl:w-auto" :class="[
                            {'w-4': !isSidebarOpen},
                            'w-4 xl:w-5 2xl:w-auto'
                            ]" alt="User Label">
                    </span>
                </div>
                <div class="userName mr-auto {{ $sidebarOpen ? '' : 'hidden' }}" :class="{'hidden': !isSidebarOpen}">
    ...
    <nav class="sidenav flex-1 px-[22px] mt-[30px] overflow-hidden hover:overflow-y-auto {{ $sidebarOpen ? '' : 'px-[12px]' }}" :class="{'px-[12px]': !isSidebarOpen}">
        <ul class="overflow-hidden">
            <li class="{{ Route::is('dashboard') ? 'active' : '' }}">
                <a
                    href="{{ route('dashboard') }}"
                    class="flex items-center px-2 py-3 space-x-2 text-base font-semibold rounded-md hover:bg-[#E9F0E9] hover:text-black active {{ $sidebarOpen ? '' : 'justify-center' }}"
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">Home</span>
                </a>
            </li>
            <li class="{{ Route::is('profile.mailbox') ? 'active' : '' }}">
                <a
                    ...
                    <!-- <span class="w-[calc(100%-30px)]" :class="{ 'lg:hidden': !isSidebarOpen }">City Mailbox</span> -->
                     <div class="sidebarLabel flex items-center justify-between w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}"
            ...
            <li class="{{ Route::is(['bank.index', 'bank.my_account', 'bank.transfer', 'bank.pay_bills', 'bank.bank_statements', 'bank.help', 'bank.manage_payee', 'bank.recurring_payment', 'bank.payment_history', 'bank.bank_statement_show', 'bank.viewStatement']) ? 'active' : '' }}">
                <a
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">Bank Account</span>
                </a>
            </li>
            <li x-data="{ open: {{ Route::is(['education.city-mall', 'education.spending-tracker*','supermarket*', 'education.npos','city-mood','education.city-hall', 'education.main-hall', 'education.civic-chamber', 'education.well-being-room']) ? 'true' : 'false' }}  }" class="relative">
                ...
                <button
                    type="button"
                    @click="open = !open"
                    class="w-full flex items-center px-2 py-3 space-x-2 text-base font-semibold rounded-md hover:bg-[#E9F0E9] hover:text-black {{ $sidebarOpen ? '' : 'justify-center' }}"
                    :class="{'justify-center': !isSidebarOpen}"
                    data-tooltip="City"
                    data-tooltip-placement="right"
                >
                    ...
                    <span
                        class="sidebarLabel w-[calc(100%-30px)] text-left {{ $sidebarOpen ? '' : 'lg:hidden' }}"
                        :class="{ 'lg:hidden': !isSidebarOpen }"
                    >
                        City
                    </span>
                    ...
                    <!-- Arrow icon -->
                    <svg
                        class="w-4 h-4 ml-auto transition-transform duration-300 {{ $sidebarOpen ? '' : 'lg:hidden' }}"
                ...
                <ul
                    x-show="open"
                    x-collapse
                    class="mt-1 pl-10 space-y-1 overflow-hidden {{ $sidebarOpen ? '' : 'hidden' }}"
            ...
            <li class="{{ Route::is(['education.educational_finance_department', 'education.index']) ? 'active' : '' }}">
                <a
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">Educational Finance Department</span>
                </a>
            </li>
            <li>
                <a
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">PISA</span>
                </a>
            </li>
            <li {{ Route::is('education.calendar') ? 'active' : '' }}>
                <a
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">Calendar</span>
                </a>
            </li>
            <li class="{{ Route::is('profile.edit') ? 'active' : '' }}">
                <a
                    ...
                    <span class="sidebarLabel w-[calc(100%-30px)] {{ $sidebarOpen ? '' : 'lg:hidden' }}" :class="{ 'lg:hidden': !isSidebarOpen }">Settings</span>
                </a>
            </li>
            <!-- Sidebar Links... -->
        </ul>
